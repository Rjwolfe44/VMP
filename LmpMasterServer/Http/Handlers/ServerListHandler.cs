...
namespace LmpMasterServer.Http.Handlers
{
    public class ServerListHandler : IHttpRequestHandler
    {
        ...
        private static Task<string> GetServerList()
        {
            return Task.Run(() =>
            {
                ...
                using (var writer = new HtmlTextWriter(stringWriter))
                {
                    ...
                    writer.RenderBeginTag(HtmlTextWriterTag.H1); writer.Write($"Vlad Multiplayer servers - Version: {LmpVersioning.CurrentVersion}"); writer.RenderEndTag();
        ...
        private static void RenderHead(HtmlTextWriter writer)
        {
            ...
            writer.Write("Vlad Multiplayer servers");
            writer.RenderEndTag();
