...
namespace LmpClient.Systems.ShareAchievements
{
    public class ShareAchievementsMessageHandler : SubSystem<ShareAchievementsSystem>, IMessageHandler
    {
        ...
        public void HandleMessage(IServerMessageBase msg)
        {
            ...
            {
                LunaLog.LogWarning("[VMP] Ignoring legacy AchievementsUpdate because persistent sync owns achievements convergence.");
                return;
            }
        ...
        private static ProgressNode ConvertByteArrayToAchievement(byte[] data, int numBytes, string progressNodeId)
        {
            ...
            {
                LunaLog.LogError($"[VMP]: Error while deserializing achievement configNode: {e}");
                return null;
            }

            if (node == null)
            {
                LunaLog.LogError("[VMP]: Error, the achievement configNode was null.");
                return null;
            }
            ...
            {
                LunaLog.LogError($"[VMP]: Error while deserializing achievement: {e}");
                return null;
            }
