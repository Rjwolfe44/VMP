...
namespace LmpClient.Systems.Flag
{
    public class FlagSystem : MessageSystem<FlagSystem, FlagMessageSender, FlagMessageHandler>
    {
        ...
        public static string LmpFlagPath { get; } = CommonUtil.CombinePaths(MainSystem.KspPath, "GameData", "VladMultiplayer", "Flags");
        ...
        public void SendFlag(string flagUrl)
        {
            ...
            if (textureInfo != null)
            {
                ...
                LunaLog.Log($"[VMP]: Uploading {Path.GetFileName(flagUrl)} flag");
        ...
        private void HandleFlag(ExtendedFlagInfo flagInfo)
        {
            ...
            if (flagTexture.LoadImage(flagInfo.FlagData))
            {
                ...
                LunaLog.Log($"[VMP]: Loaded flag {flagTexture.name}");
            }
            else
            {
                LunaLog.LogError($"[VMP]: Failed to load flag {flagInfo.FlagName}");
            }
        }
