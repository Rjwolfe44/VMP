...
namespace LmpClient.Systems.ShareContracts
{
    public class ShareContractsMessageHandler : SubSystem<ShareContractsSystem>, IMessageHandler
    {
        ...
        private static Contract ConvertByteArrayToContract(byte[] data, int numBytes)
        {
            ...
            {
                LunaLog.LogError($"[VMP]: Error while deserializing contract configNode: {e}");
                return null;
            }

            if (node == null)
            {
                LunaLog.LogError("[VMP]: Error, the contract configNode was null.");
                return null;
            }
            ...
            {
                LunaLog.LogError($"[VMP]: Error while deserializing contract: {e}");
                return null;
            }
