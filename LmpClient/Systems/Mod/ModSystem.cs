...
namespace LmpClient.Systems.Mod
{
    public class ModSystem : System<ModSystem>
    {
        ...
        public void CheckCommonStockParts()
        {
            ...
            LunaLog.Log("[VMP]: Missing parts start");
            foreach (var part in PartLoader.LoadedPartsList.Where(p => !modFile.AllowedParts.Contains(p.name)))
            {
                missingPartsCount++;
                LunaLog.Log($"[VMP]: Missing part: '{part.name}'");
            }
            LunaLog.Log("[VMP]: Missing parts end");

            LunaLog.Log("[VMP]: Missing resources start");
                ...
                .Where(r => !modFile.AllowedResources.Contains(r)))
            {
                ...
                LunaLog.Log($"[VMP]: Missing resource: '{resource}'");
            }
            LunaLog.Log("[VMP]: Missing resources end");
