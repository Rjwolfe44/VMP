    ...
    public class NetworkConnection
    {
        ...
        public static void Disconnect(string reason = "unknown")
        {
            lock (DisconnectLock)
            {
                if (MainSystem.NetworkState > ClientState.Disconnected)
                {
                    ...
                    LunaLog.Log($"[VMP]: Disconnected, reason: {reason}");
        ...
        public static void ConnectToServer(string hostname, int port, string password)
        {
            ...
            if (endpoints.Length == 0)
            {
                ...
                LunaLog.LogError($"[VMP]: Hostname resolution failed, check for typos");
        ...
        public static void ConnectToServer(IPEndPoint[] endpoints, string password)
        {
            ...
            SystemBase.TaskFactory.StartNew(() =>
            {
                ...
                foreach (var endpoint in endpoints)
                {
                    ...
                    LunaLog.Log($"[VMP]: Connecting to {endpoint.Address} port {endpoint.Port}");
                    ...
                    try
                    {
                        ...
                        {
                            LunaLog.Log("[VMP]: Starting client");
                            client.Start();
                        }
                        ...
                        if (conn == null)
                        {
                            ...
                            LunaLog.LogError($"[VMP]: Invalid connection state, connected without connection");
                        ...
                        {
                            LunaLog.Log($"[VMP]: Connected to {endpoint.Address}:{endpoint.Port}");
                            MainSystem.NetworkState = ClientState.Connected;
                            break;
                        }
                        else
                        {
                            LunaLog.Log($"[VMP]: Initial connection timeout to {endpoint.Address}:{endpoint.Port}");
