...
namespace LmpClient.Network
{
    public class NetworkServerList
    {
        ...
        public static void HandleServersList(NetIncomingMessage msg)
        {
            ...
            {
                LunaLog.LogError($"[VMP]: Invalid server list reply msg: {e}");
            }
        }
            ...
            if (Servers.TryGetValue(serverId, out var serverInfo))
            {
                ...
                else
                {
                    try
                    {
                        ...
                        LunaLog.Log($"[VMP]: Sending NAT introduction to master servers. Token: {MainSystem.UniqueIdentifier}");
                        NetworkSender.QueueOutgoingMessage(introduceMsg);
                    }
                    catch (Exception e)
                    {
                        LunaLog.LogError($"[VMP]: Error connecting to server: {e}");
                    }
                }
            }
        }
        ...
        public static void HandleNatIntroduction(NetIncomingMessage msg)
        {
            ...
            {
                LunaLog.Log($"[VMP]: Nat introduction success against {msg.SenderEndPoint}. Token: {MainSystem.UniqueIdentifier}");
                NetworkConnection.ConnectToServer(new []{ msg.SenderEndPoint }, Password);
            }
            else
            {
                LunaLog.LogError($"[VMP]: Nat introduction failed against {msg.SenderEndPoint}. Token: {MainSystem.UniqueIdentifier}");
            }
        }
    }
}
