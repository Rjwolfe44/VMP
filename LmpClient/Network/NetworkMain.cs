...
namespace LmpClient.Network
{
    public class NetworkMain
    {
        ...
        {
            LunaLog.Log($"[VMP]: Clearing {InputLockManager.lockStack.Count} control locks");
        ...
        public static void HandleDisconnectException(Exception e)
        {
            ...
            {
                LunaLog.LogError($"[VMP]: Connection error: {e.Message}, {e.InnerException}");
                NetworkConnection.Disconnect($"Connection error: {e.Message}, {e.InnerException.Message}");
            }
            else
            {
                LunaLog.LogError($"[VMP]: Connection error: {e}");
