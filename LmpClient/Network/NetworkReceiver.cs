...
namespace LmpClient.Network
{
    public class NetworkReceiver
    {
        ...
        {
            LunaLog.Log("[VMP]: Receive thread started");
            try
            {
                    ...
                    if (NetworkMain.ClientConnection.ReadMessage(out var msg))
                    {
                        switch (msg.MessageType)
                        {
                            ...
                            case NetIncomingMessageType.Data:
                                ...
                                {
                                    LunaLog.LogError($"[VMP]: Error deserializing message! {e}");
                                }
                                break;
                            case NetIncomingMessageType.StatusChanged:
                                ...
                                break;
                            default:
                                LunaLog.Log($"[VMP]: LIDGREN: {msg.MessageType} -- {msg.PeekString()}");
                                break;
                        }
            ...
            {
                LunaLog.LogError($"[VMP]: Receive thread error: {e}");
                NetworkMain.HandleDisconnectException(e);
            }
            LunaLog.Log("[VMP]: Receive thread exited");
        }
        ...
        private static void QueueMessageToSystem(IServerMessageBase msg)
        {
            switch (msg.MessageType)
            {
                ...
                case ServerMessageType.LaunchPad:
                    ...
                    break;
                default:
                    LunaLog.LogError($"[VMP]: Unhandled Message type {msg.MessageType}");
                    break;
            }
        }
    }
